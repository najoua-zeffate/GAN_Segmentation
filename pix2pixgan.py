...
Original file is located at
    https://colab.research.google.com/drive/1L1150S1vCpHTrCjlsjH7ifZz--821WUK
...
def summarize_performance(step, g_model, dataset, n_samples=3):
	...
	# save plot to file
	filename1 = '/content/plot_%06d.png' % (step+1)
	plt.savefig(filename1)
	plt.close()
	# save the generator model
	filename2 = '/content/model_%06d.h5' % (step+1)
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
    ...
    #files.download("/content/model_%06d.h5")
...
def load_images(path1,path2, size=(512,512)):
    ...
    for filename in range(0,500):
        pixels=load_img(path1 + l[filename], target_size=size)
        sat_img=img_to_array(pixels)
        src_list.append(sat_img)
    for filename in range(0,500):
        pixels = load_img(path2 + l1[filename], target_size=size)
        seg_img = img_to_array(pixels)
        tar_list.append(seg_img)
    return [asarray(src_list),asarray(tar_list)]

# dataset path
path1 = '/content/drive/MyDrive/Data_pix/img/'
path2='/content/drive/MyDrive/Data_pix/tar/'
# load dataset
...
histo=train(d_model, g_model, gan_model, dataset, n_epochs=10, n_batch=1) 
...
plt.plot(l3,label="Gen Loss")
plt.plot(l2,label="Disc Loss")
plt.xlabel("Batch")
plt.ylabel("Loss")
plt.legend()
plt.show()
...
from numpy.random import randint
model = load_model('/content/drive/MyDrive/model/model_005000.h5',compile=False)
...
path1='/content/drive/MyDrive/data_test/img/'
path2='/content/drive/MyDrive/data_test/tar/'
[src_images, tar_images] = load_images(path1,path2)
...
for ix in range(0,len(X1)):
  src_image, tar_image = X1[[ix]], X2[[ix]]
# generate image from source
  gen_image = model.predict(X1[[ix]])
# plot all three images
  plot_images(src_image, gen_image, tar_image)

import shutil
filePath = shutil.copy('/content/model_005000.h5', '/content/drive/MyDrive/model')
print(filePath)
